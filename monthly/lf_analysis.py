...
def ly_month_candidates(dates: list) -> list:
    """'전년 특정 일자 복수선택' 모드용 후보 날짜 목록. dates에 포함된 각 (연,월) 조합에 대해
    전년도 동월 1일~말일 전체를 후보로 반환한다 (예: dates가 2026-07-01~07-02 -> 2025-07-01~07-31)."""
    if not dates:
        return []
    ...
    for y, m in months:
        ly_y = y - 1
        last_day = calendar.monthrange(ly_y, m)[1]
        out += [dt.date(ly_y, m, day) for day in range(1, last_day + 1)]
